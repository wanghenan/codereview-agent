...
文件级缓存的键不仅包含文件名和规范化后的 patch，还包含一个**命名空间（namespace）**，由当前激活的 LLM 模型名和规则集哈希组成：

```
namespace = "<model>:<rules-hash>"
缓存键 = sha256( namespace : filename : normalized_patch )
```

### 为什么需要命名空间

| 场景 | 无 namespace | 有 namespace |
|------|-------------|-------------|
| 切换 LLM 模型（如 gpt-4o → glm-4-flash） | 命中旧模型的审查结果（过时） | ✅ 自动失效，重新审查 |
| 更新 OWASP 规则文件 | 命中旧规则的审查结果 | ✅ 自动失效 |
| 禁用某些规则（`--disable-rules`） | 命中含被禁规则的审查结果 | ✅ 自动失效 |

### 工作原理

- **模型名**：取自配置 `llm.model`，未指定时回退到该 provider 的默认模型
- **规则哈希**：对所有加载规则的 `(id, pattern, severity)` 排序后计算 SHA-256 前 8 位；无规则引擎时为 `no-rules`

命名空间是自动计算的，无需用户配置。这也意味着**共享缓存时，团队成员需使用相同的模型和规则配置**才能互相命中。

> 向后兼容：未指定 namespace 时默认为 `"default"`，与旧版本行为一致。
