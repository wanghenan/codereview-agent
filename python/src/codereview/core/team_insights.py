...
class TeamInsights:
    ...
    def collect_insights(self) -> dict[str, Any]:
        ...
        avg_confidence = (
            sum(r.get("confidence", 0) for r in self._reviews) / total_prs if total_prs > 0 else 0
        )
    ...
    def get_top_issues(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        for issue_type, count in sorted(
            ...
            common_description = (
                descriptions[0] if descriptions else issue_labels.get(issue_type, issue_type)
            )
    ...
    def get_developer_stats(self) -> list[dict[str, Any]]:
        ...
        for author, stats in sorted(
            authors.items(), key=lambda x: x[1].get("issues", 0), reverse=True
        ):
